...
custom_model.load_state_dict(torch.load('../Model/actionModelv1.pt'))
    ...
    while (cap.isOpened()):
      ...
      # Convert the image to PyTorch tensor
        ...
        if (first > 0.5):
          ...
          cv2.putText(Original, str(action), (20,50), cv2.FONT_HERSHEY_SIMPLEX, 3, (0,0,255), 3)

        cv2.imshow('skeletonMHI', frame_process)
        cv2.imshow('realtime view',Original)
